...
async fn led_task(
    ...
    let high_brightness = 255; // High brightness for the active LEDs
    let led_count = 97;
    let train_length = 15;

    // Define the colors for the bouncing LEDs
    let colors = [
        (high_brightness, 0, 0),
        (high_brightness, 0, 0),
        (high_brightness, 0, 0),
    ...
    let mut direction = 1; // 1 for forward, -1 for reverse
    let mut position = 0;
    ...
    while iteration_count < 10 {
        for _ in 0..led_count {
            ...
            // Set all LEDs to low brightness (background effect)
            ...
            if direction == 1 {
                position += 1;
                if position + train_length >= led_count {
